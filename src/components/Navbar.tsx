...
export default function Navbar() {
  ...
  return <header className={`site-header ${scrolled ? 'is-scrolled' : ''}`}>
    <nav className="nav container" aria-label="Main navigation">
      <a className="wordmark" href="/" onClick={close} aria-label="Emre Lofça, home">EL<span>.</span></a>
      <button className="menu-toggle" onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Menüyü aç veya kapat">
      ...
      <div className={`nav-links ${open ? 'is-open' : ''}`}>
        ...
        <div className="nav-actions">
          ...
          <button className="language-switch" onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} aria-label="Change language">
            <span className={lang === 'tr' ? 'active' : ''}>TR</span><i /><span className={lang === 'en' ? 'active' : ''}>EN</span>
          </button>
          <a className="button button-small" href={cvPath} download><Download size={16} />{t.nav.downloadCV}</a>
        </div>
      </div>
    </nav>
  </header>;
